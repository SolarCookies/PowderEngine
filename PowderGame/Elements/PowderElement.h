...
class PowderElement {
public:
	PowderElement(const std::string& scriptPath = "", MonoDomain* domain = nullptr) { init(scriptPath, domain); }
	virtual void init(const std::string& scriptPath, MonoDomain* domain) { return; } // Called when the element is created
